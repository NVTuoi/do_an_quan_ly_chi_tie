...
class StatisticalServices {
  Future<GetStatistical> getStatistical() async {
    ...
    CollectionReference categorySpendCollection = FirebaseFirestore.instance
        .collection(CollectionName.categorySpend.name);
    CollectionReference categoryCollectCollection = FirebaseFirestore.instance
        .collection(CollectionName.categoryCollect.name);
    ...
    for (var item in dataSumDaySpendAllLats.docs) {
      var e = CostSpend.formJson(item.data() as Map<String, dynamic>)
        ..id = item.id;
      if (e.idUser == idUser &&
          e.dateTime!.day.toString() == (DateTime.now().day - 1).toString()) {
    ...
    for (var item in dataSumYearSpendAllLats.docs) {
      var e = CostSpend.formJson(item.data() as Map<String, dynamic>)
        ..id = item.id;
      if (e.idUser == idUser &&
          e.dateTime!.year.toString() == (DateTime.now().year - 1).toString()) {
    ...
    GroupBy groupSumCollectDay =
        GroupBy(name: 'Tổng thu ', money: sumDayCollect);
    ...
    GroupBy groupSumCollectMoth =
        GroupBy(name: 'Tổng thu ', money: sumMonthCollect);
    ...
    GroupBy groupSumCollectYear =
        GroupBy(name: 'Tổng thu ', money: sumYearCollect);
    ...
    for (var categorySpend in dataGroupSpendCategory.docs) {
      ...
      if (ktDay == true) {
        if (sumDaySpendMoney != 0) {
          GroupBy groupBySpend =
              GroupBy(name: category.name, money: sumDaySpendMoney);
    ...
    for (var categorySpend in dataGroupSpendCategory.docs) {
      ...
      if (ktMoth == true) {
        if (sumMothMoney != 0) {
          GroupBy groupBySpend =
              GroupBy(name: category.name, money: sumMothMoney);
    ...
    for (var categorySpend in dataGroupSpendCategory.docs) {
      ...
      if (ktYear == true) {
        if (sumYearMoney != 0) {
          GroupBy groupBySpend =
              GroupBy(name: category.name, money: sumYearMoney);
    ...
    for (var categoryCollect in dataGroupCollectCategory.docs) {
      var category = CategoryCollect.fromJson(
          categoryCollect.data() as Map<String, dynamic>)
        ..id = categoryCollect.id;
      if (ktDayCollect == true) {
        bool ktDayCollect = false;
        sumDayCollectMoney = 0;
      }
      for (var costCollect in dataGroupCollectCost.docs) {
        var collect =
            CostCollect.formJson(costCollect.data() as Map<String, dynamic>)
              ..id = costCollect.id;
        if (category.idUser == idUser &&
      ...
      if (ktDayCollect == true) {
        if (sumDayCollectMoney != 0) {
          GroupBy groupByCollect =
              GroupBy(name: category.name, money: sumDayCollectMoney);
    ...
    for (var categoryCollect in dataGroupCollectCategory.docs) {
      ...
      for (var costCollect in dataGroupCollectCost.docs) {
        ...
        if (category.idUser == idUser &&
            collect.idUser == idUser &&
            category.id.toString() == collect.idCategoryCollect.toString() &&
            collect.dateTime!.month.toString() ==
                DateTime.now().month.toString()) {
      ...
      if (ktMothCollect == true) {
        if (sumMothCollectMoney != 0) {
          GroupBy groupByCollect =
              GroupBy(name: category.name, money: sumMothCollectMoney);
    ...
    for (var categoryCollect in dataGroupCollectCategory.docs) {
      ...
      for (var costCollect in dataGroupCollectCost.docs) {
        ...
        if (category.idUser == idUser &&
            collect.idUser == idUser &&
            category.id.toString() == collect.idCategoryCollect.toString() &&
            collect.dateTime!.year.toString() ==
      ...
      if (ktMothCollect == true) {
        if (sumYearCollectMoney != 0) {
          GroupBy groupByCollect =
              GroupBy(name: category.name, money: sumYearCollectMoney);
          groupByYearCollect.add(groupByCollect);
        }
      }
    }

    GetStatistical value = GetStatistical(
