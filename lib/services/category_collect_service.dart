...
class CategoryCollectService {
  //tạo danh mục thu
  Future<dynamic> createCategoryCollect({
    required CategoryCollect categoryCollect,
  }) async {
    ...
    for (var category in nameCategoryCollect.docs) {
      var categoryName =
          CategoryCollect.fromJson(category.data() as Map<String, dynamic>)
            ..id = category.id;
      if (categoryName.name == categoryCollect.name &&
          categoryName.idUser == idUser) {
        print('Đã có !');
      }
    }
...
//Cập nhật danh mục thu
  Future<dynamic> updateCategoryCollect(
    ...
    CollectionReference categoryCollectCollection = FirebaseFirestore.instance
        .collection(CollectionName.categoryCollect.name);
    await categoryCollectCollection.doc(categoryCollect.id).update(
          categoryCollect.toJson(),
        );
    //Cập nhật khoản thu
    var idUser = await SecureStorage().getString(key: SecureStorage.userId);
    ...
    for (var item in data.docs) {
      ...
      if (e.idUser == idUser && e.idCategoryCollect == categoryCollect.id) {
        await costCollectCollection
            .doc(e.id)
            .update({'nameCategoryCollect': categoryCollect.name});
      }
    }
  }
  ...
  Future<dynamic> deleteCategoryCollect(
    ...
    for (var item in data.docs) {
      ...
      if (e.idUser == idUser && e.idCategoryCollect == categoryCollect.id) {
        await costCollectCollection
            .doc(e.id)
            .update({'nameCategoryCollect': "Null", 'idCategorySpend': 'Null'});
      }
    }
  }
}
